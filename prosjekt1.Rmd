...
\item To estimate ${\pi}_k$ we consider the proportion of observations coming from class $k$, that is, $\hat{\pi}_k = \frac{n_k}{n}$. We compute $\hat{\pi}_1 = 59 / 130 = 0.4538$ and $\hat{pi}_2 = 71 / 130 = 0.5461$. To estimate ${\mu_k}$ we consider the estimated mean of points coming from class $k$, that is $\hat{\mu}_k = \frac{1}{n_k} \sum_{i, y_i = k} x_i$, which we compute to be: $\hat{\mu}_1 = (17.0373, 5.5283)^T$, $\hat{\mu}_2 = (20.2380, 3.0866)^T$. To estimate $\Sigma$ we consider the estimated variance for each class, $\hat{\Sigma}_k := \frac{1}{n_k - 1}\sum_{i, y_i = k} (X_i - \hat{\mu}_k)(X_i - \hat{\mu}_k)^T$, and compute: 
\[
...
X <- data.matrix(wine)
n = 130; n1 = 59; n2 = 71
X1 <- X[1:59,2:3]; X2 <- X[60:n,2:3]
mu1hat <- t(X1) %*% rep(1, n1) / n1; mu2hat <- t(X2) %*% rep(1, n2) / n2
...
\item Let $\hat{\delta}_k(x) := \log(\hat{\pi}_k) - \frac{1}{2}(x - \hat{\mu}_k)^T{\hat{\Sigma}}^{-1}(x - \hat{\mu_k}))$, then this descisionboundary is given by

\[
\hat{Pr}(Y = 1 | X) = \frac{\pi_1\hat{f}_1(x)}{\sum_{i = 1}^{2}\pi_i \hat{f}_i(x)} = \frac{\exp(\hat{\delta_1}(x))}{\sum_{i = 1}^2\exp(\hat{\delta_i}(x))} = 0.5
\]

which gives

\[
\exp(\hat{\delta}_2(x)) = \exp(\hat{\delta}_1(x)),
\]
...
\[
\log(\hat{\pi_1}) + \frac{1}{2}{\hat{\mu_1}}^T{\Sigma}^{-1}\hat{\mu_1} - {\hat{\mu_1}}^T{\hat{\Sigma}}^{-1}x = \log(\hat{\pi_2}) + \frac{1}{2}{\hat{\mu_2}}^T{\Sigma}^{-1}\hat{\mu_2} - {\hat{\mu_2}}^T{\hat{\Sigma}}^{-1}x
\]

that is

\[
\frac{1}{2}({\hat{\mu}_1}^T {\hat{\Sigma}}^{-1}\hat{\mu_1} - {\hat{\mu}_2}^T {\hat{\Sigma}}^{-1}\hat{\mu_2}) + \log\bigg(\frac{\hat{\pi}_1}{\hat{\pi}_2}\bigg) + ({\hat{\mu}_2}^T{\hat{\Sigma}}^{-1} - {\hat{\mu}_1}^T{\hat{\Sigma}}^{-1})x = 0
\]

Inserting our obtained estimates we get the boudary

\[
y = x
\]

\item The descision boundary with both the training and test observations is shown below

\end{itemize}

```{r, echo = FALSE}
g1 = ggplot(data=train,aes(x=x1, y=x2, colour=y)) + geom_point(pch = 1)
g1 = g1 + geom_point(data = test, pch = 3) + geom_abline(slope=slope,intercept=intercept)+ggtitle("train and test and logistic boundary")
#g1
```
...
```{r}
wine_lda <- lda(y ~ x1 + x2, data = wine, prior = c(0.4538, 1 - 0.4538))

```

\begin{itemize}

\item The confusion table is show below
...
\item The most important diffence in regard to using LDA or QDA would be that with QDA we expect the variance of the classes to be different, and hence use different covariance matrices.
